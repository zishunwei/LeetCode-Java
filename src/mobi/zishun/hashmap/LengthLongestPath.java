...
public class LengthLongestPath {
    ...
    public int lengthLongestPath(String input) {
        ...
        for (String line : lines) {
            ...
            // 需要去掉'\t'占用的个数（即为depth）
            hashMap.put(depth, hashMap.getOrDefault(depth - 1, 0) + line.length() - depth);
            // 此行中有'.'的情况，说明统计到最后的文件了，参与计算结果
            if (flag) {
                // 每层都要添加depth个'/'
                res = Math.max(res, hashMap.get(depth) + depth);
            }
        }
        return res;
    }
